...
@section('content')
<div class="mb-8">
    <h1 class="text-2xl font-bold tracking-tight text-slate-900">Fine Management</h1>
    <p class="mt-1 text-sm text-slate-500">Track student late return fees and toggle payment status</p>
</div>
                ...
                @forelse($fines as $fine)
                    <tr class="transition hover:bg-slate-50">
                        <td class="whitespace-nowrap px-6 py-4 font-semibold text-slate-900">
                            {{ $fine->user->name }}
                            <span class="block text-xxs font-normal text-slate-400">{{ $fine->user->email }}</span>
                        </td>
                        ...
                        <td class="whitespace-nowrap px-6 py-4 font-bold text-rose-600">
                        ...
                        <td class="whitespace-nowrap px-6 py-4 text-right text-xs">
                            <form method="post" action="{{ route('admin.fines.toggle', $fine) }}">
                                @csrf 
                                @method('patch')
                                <button type="submit" class="inline-flex items-center justify-center rounded-lg border border-slate-200 bg-white px-3 py-1.5 font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 hover:text-slate-900">
                                    Toggle Paid
                                </button>
                            </form>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-8 text-center text-slate-500">
                            No fines registered in the database.
                        </td>
                    </tr>
                @endforelse
            </tbody>
